...
import { VscGenerationRequest, JetbrainsGenerationRequest } from "./types"
...
export class CommitMessageProvider implements vscode.Disposable {
	...
	constructor(
		...
		this.vscodeAdapter = new VSCodeCommitMessageAdapter(context, outputChannel, this.generator)
	...
	private async handleJetBrainsCommand(...args: JetbrainsGenerationRequest): Promise<CommitMessageResult> {
		...
		// We need to extract from the first element
		const firstArg = args[0]

		let workspacePath: string
		let selectedFiles: string[]

		if (Array.isArray(firstArg) && firstArg.length === 2) {
			// Handle nested array format: [[workspacePath, selectedFiles]]
			;[workspacePath, selectedFiles] = firstArg
		} else {
			// Handle direct format: [workspacePath, selectedFiles] (fallback)
			;[workspacePath, selectedFiles] = args
		}
